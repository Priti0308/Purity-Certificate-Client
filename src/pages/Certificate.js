...
const Certificate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('❌ Error submitting certificate:', err.response?.data || err.message);
  ...
  const convertToWords = (num) => {
    ...
    const integerPart = parts[0].split('').map(d => map[d] || '').join(' ');
    const decimalPart = parts[1]
      ? parts[1].split('').map(d => map[d] || '').join(' ')
      : '';
  ...
  const handleDownloadPDF = async (cert) => {
    ...
    container.innerHTML = `
      <div style="font-family: Arial; font-size: 12px; border: 1px solid black; padding: 20px; width: 700px; margin: auto;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 10px;" border="1">
          ...
          <tr><td><strong>In Words</strong></td><td colspan="3">${convertToWords(cert.fineness)} %</td></tr>
        </table>
    ...
    <div className="container py-4">
      ...
      <form onSubmit={handleSubmit} className="card shadow p-4 mb-5">
        <div className="row g-3">
          {[
            { label: 'Serial No', name: 'serialNo', placeholder: 'e.g., SRJ-001' },
            { label: 'Customer Name', name: 'name', placeholder: 'e.g., Ravi Kumar' },
            { label: 'Item', name: 'item', placeholder: 'e.g., Silver Ring' },
            { label: 'Fineness (%)', name: 'fineness', placeholder: 'e.g., 91.6' },
            { label: 'Gross Weight (g)', name: 'grossWeight', placeholder: 'e.g., 15.2' },
        ...
        <div className="text-end mt-4">
          <button type="submit" className="btn btn-success">
            <FaPlus className="me-2" /> {editId ? 'Update' : 'Submit'} Certificate
          </button>
        </div>
      </form>
